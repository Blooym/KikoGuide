...
internal class KikoUISettings : IDisposable
{
    ...
    // <summary>
    // Instantiates a new KikoUISettings.
    // </summary>
    public KikoUISettings(Configuration configuration)
    {
    ...
    // <summary>
    // Disposes of the KikoUISettings and any resources it uses.
    // </summary>
    public void Dispose()
    { }
    ...
    // <summary>
    // Draws all UI elements within the settings UI.
    // </summary>
    ...
    private void DrawSettingsWindow()
    {
        ...
        if (!KikoUIState.settingsVisible) return;
        ...
        if (ImGui.Begin(String.Format(Loc.Localize("UI.Settings.Title", "{0} - Settings"), PStrings.PluginName), ref KikoUIState.settingsVisible, ImGuiWindowFlags.NoResize))
        {
            ...
            if (ImGui.BeginTabItem("Debug"))
            {
                ...
                ImGui.TextWrapped($"Name: {KikoUIState.SelectedDuty?.Name ?? "None"}");
                ImGui.NextColumn();
                ImGui.TextWrapped($"Expansion: {Enum.GetName(typeof(Expansion), KikoUIState.SelectedDuty?.Expansion ?? -1)}");
                ImGui.NextColumn();
                ImGui.TextWrapped($"Level: {KikoUIState.SelectedDuty?.Level ?? 0}");
                ImGui.NextColumn();
                ImGui.TextWrapped($"Bosses: {KikoUIState.SelectedDuty?.Bosses?.Count ?? 0}");
                ImGui.NextColumn();
                ImGui.TextWrapped($"QuestUnlockID: {KikoUIState.SelectedDuty?.UnlockQuestID ?? 0}");
                ImGui.NextColumn();
                ImGui.TextWrapped($"TerritoryID: {KikoUIState.SelectedDuty?.TerritoryID ?? 0}");
                ImGui.EndChild();
                ...
                if (ImGui.Button("Export"))
                {
                    try
                    {
                        ...
                        Service.Toasts.ShowQuest("Localization exported successfully.");
                    }
                    catch (Exception e)
                    {
